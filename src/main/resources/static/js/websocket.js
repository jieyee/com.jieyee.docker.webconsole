...
const connect = () => {
  ...
  let socket = new SockJS('/wsendpoint');
  ...
  stompClient.connect(
    ...
    (frame) => {
      ...
      // 订阅广播消息
      const subscription_broadcast = stompClient.subscribe('/topic/broadcast',
        (response) => {
          if (response.body) {
      ...
      // 订阅私人消息
      const subscription_personal = stompClient.subscribe('/user/' + sessionId + '/personal', (response) => {
        if (response.body) {
      ...
      // 订阅异常消息
      const subscription_errors = stompClient.subscribe('/user/' + sessionId + '/errors', (response) => {
        if (response.body) {
...
document.getElementById("chat").onkeydown = (KeyboardEvent) => {
  if (KeyboardEvent.keyCode === 13) {
    ...
    if (message !== "") {
      ...
      stompClient.send("/ws/chat", headers, JSON.stringify(body));
...
document.getElementById("speak").onkeydown = (KeyboardEvent) => {
  if (KeyboardEvent.keyCode === 13) {
    ...
    if (message !== "") {
      ...
      stompClient.send("/ws/speak", headers, JSON.stringify(body));
